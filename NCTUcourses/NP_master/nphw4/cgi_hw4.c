/******
 *
 * 'NP hw4 CGI program with SOCKS client modified from hw3'
 * by headhsu
 *
 * gcc -lfcgi -o cgi_hw4.fcgi cgi_hw4.c
 *
 ******/
...
#define QRYSTR_MAX_LEN 2048
#define BATCHFILE_NAME_MAX_LEN 64
        ...
        while(activeConnNum > 0) {
            ...
            for(i = 0; i < connNum; ++i) {
                ...
                if(conn[i].status == CONNECTING && ((FD_ISSET(conn[i].connfd, &readSet) || FD_ISSET(conn[i].connfd, &writeSet)))) {
                    ...
                    if(res < 0 || error != 0) {
                        if(conn[i].sip != NULL && conn[i].sport != NULL) {
                            printf("error: fd: %d connect to SOCKS server %s:%s failed (return: %d, error: %d, errno: %d)<br>\n",conn[i].connfd, conn[i].sip, conn[i].sport, res, error, errno);fflush(stdout);
                        }
                        else {
                            printf("error: fd: %d connect to %s:%s failed (return: %d, error: %d, errno: %d)<br>\n",conn[i].connfd, conn[i].ip, conn[i].port, res, error, errno);fflush(stdout);
                        }
                        DisconnectConn(i);
                    }
                    else if(conn[i].sip != NULL && conn[i].sport != NULL) {
                        /*** SOCKS client connection ***/

                        //printf("fd: %d connect to SOCKS server %s:%s successfully<br>\n",conn[i].connfd, conn[i].sip, conn[i].sport);fflush(stdout);
                        conn[i].status = SENDREQUEST;

                        if(FD_ISSET(conn[i].connfd, &writeSet)) {

                            /*** send SOCKS client request ***/
                            SendRequest(i);
                            conn[i].status = WAITFORREPLY;
                        }
                    }
                    else {
                        /*** normal connection ***/

                        //printf("fd: %d connect to %s:%s successfully<br>\n",conn[i].connfd, conn[i].ip, conn[i].port);fflush(stdout);
                ...
                else if(conn[i].status == SENDREQUEST && FD_ISSET(conn[i].connfd, &writeSet)) {

                    /*** send SOCKS client request ***/
                    SendRequest(i);
                    conn[i].status = WAITFORREPLY;
                }
                else if(conn[i].status == WAITFORREPLY && FD_ISSET(conn[i].connfd, &readSet)) {

                    /*** read SOCKS server response ***/
                    if(ReadReply(i)) conn[i].status = READING;
                    else DisconnectConn(i);
                }
...
void SetConnVar() {
    ...
    for(i = 0; i < paramNum; ++i) {
        ...
        if(*(param[i].paramKey) == 'h') {
            ...
            for(j = 0; j < connNum; ++j) {
                if(*(param[i].paramKey + 1) == *(conn[j].no)) break;
            }
            if(j == connNum) InitialConn(param[i].paramKey + 1);
        ...
        else if(*(param[i].paramKey) == 's' && *(param[i].paramKey + 1) == 'h') {
            /*** set no. and SOCKS server ip ***/

            for(j = 0; j < connNum; ++j) {
                if(*(param[i].paramKey + 2) == *(conn[j].no)) break;
            }
            if(j == connNum) InitialConn(param[i].paramKey + 2);

            conn[j].sip = param[i].paramValue;
        }
        else if(*(param[i].paramKey) == 's' && *(param[i].paramKey + 1) == 'p') {
            /*** set no. and SOCKS server port ***/
            ...
            conn[j].sport = param[i].paramValue;
        }
    }
    //ListAllConnVar();
    return;
...
/*** initial a conn (an element in an array of struct CONN) when read a new no. ***/
void InitialConn(char* no) {
    conn[connNum].connfd = -1;
    conn[connNum].status = DONE;
    conn[connNum].no = no;
    conn[connNum].ip = NULL;
    conn[connNum].port = NULL;
    conn[connNum].batchFile = NULL;
    conn[connNum].fp = NULL;
    conn[connNum].sip = NULL;
    conn[connNum].sport = NULL;
    ++connNum;
    return;
}
...
void CreateConnectionForConn() {
    ...
    for(i = 0; i < connNum; ++i) {
        ...
        if(conn[i].sip != NULL && conn[i].sport != NULL) {
            conn[i].conn_addr.sin_addr.s_addr = inet_addr(conn[i].sip);
            conn[i].conn_addr.sin_port = htons(atoi(conn[i].sport));
        }
        else {
        ...
        if(res < 0 && errno!=EINPROGRESS) {
            if(conn[i].sip != NULL && conn[i].sport != NULL) {
                printf("error: connect to SOCKS server: %s:%s failed<br>\n", conn[i].sip, conn[i].sport);fflush(stdout);
            }
            else {
        ...
        /*
        if(conn[i].sip != NULL && conn[i].sport != NULL) {
            printf("connect to SOCKS server: %s:%s!<br>\n", conn[i].sip, conn[i].sport);fflush(stdout);
        }
        else {
...
/*** send SOCKS client request to SOCKS server ***/
void SendRequest(int i) {
    unsigned short int port = htons(atoi(conn[i].port));
    unsigned long int ip = inet_addr(conn[i].ip);
    struct SOCKS4_Request request;
    request.VN = (unsigned char)4;
    request.CD = (unsigned char)1;
    memcpy(request.DST_PORT, &port, sizeof(unsigned char) * 2);
    memcpy(request.DST_IP, &ip, sizeof(unsigned char) * 4);
    strcpy(request.USER_ID, "Yen-Chun Hsu");
    write(conn[i].connfd, &request, sizeof(request));
    return;
}

/*** read SOCKS server response to SOCKS client ***/
int ReadReply(int i) {
    struct in_addr ip;
    unsigned short int port;
    struct SOCKS4_Reply reply;
    read(conn[i].connfd, &reply, sizeof(reply));
    memcpy(&ip.s_addr, reply.DST_IP, sizeof(unsigned long int));
    memcpy(&port, reply.DST_PORT, sizeof(unsigned short int));
    //printf("VN: %d, CD: %d, DST IP: %s, DST PORT: %d<br>\n", (int)reply.VN, (int)reply.CD, inet_ntoa(ip), ntohs(port));fflush(stdout);

    if(reply.CD == 90) {
        //printf("GRANT CONNECT to SOCKS server %s:%d<br>\n", inet_ntoa(ip), ntohs(port));fflush(stdout);
        return 1;
    }
    else {
        printf("REJECT CONNECT to SOCKS server %s:%d<br>\n", inet_ntoa(ip), ntohs(port));fflush(stdout);
        return 0;
    }
}
...
void ListAllConnVar() {
    ...
    for(; i < connNum; ++i) {
        ...
        printf("batch file(f%c): %s, ",  *(conn[i].no), conn[i].batchFile);fflush(stdout);
        printf("SOCKS ip(sh%c): %s, ", *(conn[i].no), conn[i].sip);fflush(stdout);
        printf("SOCKS port(sp%c): %s]<br>\n", *(conn[i].no), conn[i].sport);fflush(stdout);
    }
    return;
}
