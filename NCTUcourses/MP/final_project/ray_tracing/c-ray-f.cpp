...
void sig_check(uint32_t *pixels)
{ 
  ...
  {                
    myrand(&sig, (pixels[i] >> RSHIFT) & 0xff);
    myrand(&sig, (pixels[i] >> GSHIFT) & 0xff);
    myrand(&sig, (pixels[i] >> BSHIFT) & 0xff);
  }
...
int main(int argc, char **argv) {
        ...
        for(i=0; i<NRAN; i++) urand[i].x = (double)myrand(&seed, i) / RAND_MAX - 0.5;
        for(i=0; i<NRAN; i++) urand[i].y = (double)myrand(&seed, i) / RAND_MAX - 0.5;
        for(i=0; i<NRAN; i++) irand[i] = (int)(NRAN * ((double)myrand(&seed, i) / RAND_MAX));
...
void* findNearest(void* argtest){
        struct args *arg = (struct args*)argtest;
        /* find the nearest intersection ... */
        int i;
        for(i=arg->start; i<arg->end; i++){
                if(ray_sphere(&(arg->iters[i]), *(arg->ray), &(arg->sp))) {
                        if(!arg->nearest_obj || arg->sp.dist < arg->nearest_sp->dist) {
                                arg->nearest_obj = &arg->iters[i];
                                arg->nearest_sp = &arg->sp;
                        }
                }
        }
...
struct vec3 trace(struct ray ray, int depth) {
        ...
        struct sphere *iterary[200]; 
        struct sphere *nearest_objs[NUM_THREADS];
        struct spoint nearest_sps[NUM_THREADS];


        //Init iterary to point iter_list
        int count=0;
        while(iter){
            iterary[count]=iter;
            iter = iter->next;
        ...
        thread_array = (pthread_t*)malloc(sizeof(thread_array[0])*NUM_THREADS);
        pthread_attr_t attr;
        pthread_attr_init(&attr);
        pthread_attr_setdetachstate(&attr, PTHREAD_CREATE_JOINABLE);
        struct args* args;
        args = (struct args*)malloc(sizeof(args[0])*NUM_THREADS);
        ...
        int block = count/NUM_THREADS;


        int g;
        for(g=0; g<NUM_THREADS; g++){
            args[g].id = g;
            args[g].start = g*block;
            if(g==NUM_THREADS-1) args[g].end = count;
            else args[g].end = args[g].start + block;
            args[g].iters = iterary[0];
            args[g].ray = &ray;
            args[g].nearest_sp = &nearest_sps[g];
            nearest_objs[g]=0;
            args[g].nearest_obj = nearest_objs[g];

            
            pthread_create(&(thread_array[g]), &attr, (void* (*)(void*))findNearest, (void *)(args+g) );
        }


        for(g=0; g<NUM_THREADS; g++){
            pthread_join((thread_array[g]), NULL);
            if(!nearest_obj || nearest_sps[g].dist  < nearest_sp.dist){
                nearest_obj = nearest_objs[g];
                nearest_sp = nearest_sps[g];   
            }
        }
