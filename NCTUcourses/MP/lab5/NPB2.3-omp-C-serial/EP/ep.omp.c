/*--------------------------------------------------------------------
  ...
  NAS Parallel Benchmarks 2.3 C versions - EP

  This benchmark is an C version of the NPB EP code.
...
int main(int argc, char **argv) {
    ...
    /*
       ...
       */
    printf("\n\n NAS Parallel Benchmarks 2.3 C version"
            " - EP Benchmark\n");
...
#pragma omp parallel for\
    private(kk,t1,t2,ik,t3,x1,x2,t4,i,x)\
    reduction(+:sx,sy)
    for (k = 1; k <= np; k++) {
        kk = k_offset + k;
        t1 = S;
