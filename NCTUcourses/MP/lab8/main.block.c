...
void* analyzer(void* arg)
{
    ...
    int interval_stock_num = stock_num/thread_num;
    int start_stock = interval_stock_num*threadid;
    int end_stock = start_stock + interval_stock_num;
    if(end_stock > stock_num)
        end_stock = stock_num;

    printf("threadid:%d with start_stock:%d & end_stock:%d\n", threadid, start_stock, end_stock); fflush(stdout);
    pthread_barrier_wait(&bar);

    for(i = 0; i < day_num; i++)
    {
        for(j = start_stock; j < end_stock; ++j)
        {
